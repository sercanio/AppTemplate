...
public class StatisticsControllerUnitTests
{
  ...
  #region GetNotificationStatistics Tests

  [Fact]
  public async Task GetNotificationStatistics_WithSuccessfulResult_ReturnsOkResult()
  {
    // Arrange
    var cancellationToken = CancellationToken.None;
    var notificationStats = new NotificationStatisticsResponse(
        TotalNotifications: 200,
        UnreadNotifications: 45,
        NotificationsToday: 12,
        NotificationsByType: new Dictionary<string, int> { { "Info", 150 }, { "Warning", 30 }, { "Error", 20 } },
        NotificationsByChannel: new Dictionary<string, int> { { "Email", 180 }, { "Push", 20 } });

    var result = Result.Success(notificationStats);

    _mockSender.Setup(x => x.Send(It.IsAny<GetNotificationStatisticsQuery>(), cancellationToken))
        .ReturnsAsync(result);

    // Act
    var actionResult = await _controller.GetNotificationStatistics(cancellationToken);

    // Assert
    var okResult = Assert.IsType<OkObjectResult>(actionResult);
    var returnedResponse = Assert.IsAssignableFrom<NotificationStatisticsResponse>(okResult.Value);
    Assert.Equal(200, returnedResponse.TotalNotifications);
    Assert.Equal(45, returnedResponse.UnreadNotifications);
    Assert.Equal(12, returnedResponse.NotificationsToday);
    Assert.Equal(3, returnedResponse.NotificationsByType.Count);
    Assert.Equal(2, returnedResponse.NotificationsByChannel.Count);
    _mockSender.Verify(x => x.Send(It.IsAny<GetNotificationStatisticsQuery>(), cancellationToken), Times.Once);
  }

  [Fact]
  public async Task GetNotificationStatistics_WithFailedResult_ReturnsErrorResponse()
  {
    // Arrange
    var cancellationToken = CancellationToken.None;
    var result = Result<NotificationStatisticsResponse>.Error("Notification statistics unavailable");

    _mockSender.Setup(x => x.Send(It.IsAny<GetNotificationStatisticsQuery>(), cancellationToken))
        .ReturnsAsync(result);

    var errorResult = new BadRequestObjectResult("Notification statistics unavailable");
    _mockErrorHandlingService.Setup(x => x.HandleErrorResponse(It.Is<Result<NotificationStatisticsResponse>>(r => !r.IsSuccess)))
        .Returns(errorResult);
    ...
    // Assert
    Assert.IsType<BadRequestObjectResult>(actionResult);
    _mockErrorHandlingService.Verify(x => x.HandleErrorResponse(It.Is<Result<NotificationStatisticsResponse>>(r => !r.IsSuccess)), Times.Once);
  }

  [Fact]
  public async Task GetNotificationStatistics_WithNullResult_ReturnsErrorResponse()
  {
    // Arrange
    var cancellationToken = CancellationToken.None;

    _mockSender.Setup(x => x.Send(It.IsAny<GetNotificationStatisticsQuery>(), cancellationToken))
        .ReturnsAsync((Result<NotificationStatisticsResponse>)null);

    var errorResult = new BadRequestObjectResult("Failed to retrieve notification statistics.");
    _mockErrorHandlingService.Setup(x => x.HandleErrorResponse(It.Is<Result<NotificationStatisticsResponse>>(r => !r.IsSuccess)))
        .Returns(errorResult);
  ...
  #region GetSystemStatistics Tests

  [Fact]
  public async Task GetSystemStatistics_WithSuccessfulResult_ReturnsOkResult()
  {
    // Arrange
    var cancellationToken = CancellationToken.None;
    var systemStats = new SystemStatisticsResponse(
        TotalApiRequests: 10000,
        RateLimitedRequests: 25,
        AverageResponseTime: 150.5,
        RequestsByEndpoint: new Dictionary<string, int> { { "/api/users", 5000 }, { "/api/roles", 3000 }, { "/api/statistics", 2000 } },
        TotalErrors: 50);

    var result = Result.Success(systemStats);

    _mockSender.Setup(x => x.Send(It.IsAny<GetSystemStatisticsQuery>(), cancellationToken))
        .ReturnsAsync(result);

    // Act
    var actionResult = await _controller.GetSystemStatistics(cancellationToken);

    // Assert
    var okResult = Assert.IsType<OkObjectResult>(actionResult);
    var returnedResponse = Assert.IsAssignableFrom<SystemStatisticsResponse>(okResult.Value);
    Assert.Equal(10000, returnedResponse.TotalApiRequests);
    Assert.Equal(25, returnedResponse.RateLimitedRequests);
    Assert.Equal(150.5, returnedResponse.AverageResponseTime);
    Assert.Equal(3, returnedResponse.RequestsByEndpoint.Count);
    Assert.Equal(50, returnedResponse.TotalErrors);
    _mockSender.Verify(x => x.Send(It.IsAny<GetSystemStatisticsQuery>(), cancellationToken), Times.Once);
  }

  [Fact]
  public async Task GetSystemStatistics_WithFailedResult_ReturnsErrorResponse()
  {
    // Arrange
    var cancellationToken = CancellationToken.None;
    var result = Result<SystemStatisticsResponse>.Error("System statistics unavailable");

    _mockSender.Setup(x => x.Send(It.IsAny<GetSystemStatisticsQuery>(), cancellationToken))
        .ReturnsAsync(result);

    var errorResult = new BadRequestObjectResult("System statistics unavailable");
    _mockErrorHandlingService.Setup(x => x.HandleErrorResponse(It.Is<Result<SystemStatisticsResponse>>(r => !r.IsSuccess)))
        .Returns(errorResult);
    ...
    // Assert
    Assert.IsType<BadRequestObjectResult>(actionResult);
    _mockErrorHandlingService.Verify(x => x.HandleErrorResponse(It.Is<Result<SystemStatisticsResponse>>(r => !r.IsSuccess)), Times.Once);
  }

  [Fact]
  public async Task GetSystemStatistics_WithNullResult_ReturnsErrorResponse()
  {
    // Arrange
    var cancellationToken = CancellationToken.None;

    _mockSender.Setup(x => x.Send(It.IsAny<GetSystemStatisticsQuery>(), cancellationToken))
        .ReturnsAsync((Result<SystemStatisticsResponse>)null);

    var errorResult = new BadRequestObjectResult("Failed to retrieve system statistics.");
    _mockErrorHandlingService.Setup(x => x.HandleErrorResponse(It.Is<Result<SystemStatisticsResponse>>(r => !r.IsSuccess)))
        .Returns(errorResult);
