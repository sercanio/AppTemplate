...
public class AccountsControllerUnitTests
{
  ...
  public async Task Login_WithValidCredentials_ReturnsOkResult()
  {
    ...
    _mockUserManager.Setup(x => x.FindByEmailAsync(request.LoginIdentifier))
        .ReturnsAsync(user);
  ...
  public async Task Login_WithInvalidUser_ReturnsBadRequest()
  {
    ...
    _mockUserManager.Setup(x => x.FindByEmailAsync(request.LoginIdentifier))
        .ReturnsAsync((IdentityUser)null);
    _mockUserManager.Setup(x => x.FindByNameAsync(request.LoginIdentifier))
        .ReturnsAsync((IdentityUser)null);
  ...
  [Fact]
  public async Task GetCurrentUser_WithFailedResult_ReturnsErrorResponse()
  {
    // Arrange
    var result = Result<UserResponse>.Error("User not found");

    _mockSender.Setup(x => x.Send(It.IsAny<GetLoggedInUserQuery>(), It.IsAny<CancellationToken>()))
        .ReturnsAsync(result);

    var errorResult = new NotFoundObjectResult("User not found");
    _mockErrorHandlingService.Setup(x => x.HandleErrorResponse(It.Is<Result<UserResponse>>(r => !r.IsSuccess)))
        .Returns(errorResult);
    ...
    _mockErrorHandlingService.Verify(x => x.HandleErrorResponse(It.Is<Result<UserResponse>>(r => !r.IsSuccess)), Times.Once);
  }

  [Fact]
  public async Task Logout_WithAuthenticatedUser_ReturnsOkResult()
  {
    // Arrange
